...
function resolveClassConflicts(defaultClasses: string, passedClasses: string) {
  ...
  const processedClassList = defaultClassList?.map((defaultClass) => {
    ...
    if (dynamicClassPattern.test(defaultClass)) {
      ...
      const passedClass = passedClassList.find(
        (cls) =>
          dynamicClassPattern.test(cls) &&
          cls.includes(
            defaultClass.match(dynamicClassPattern)![0].split("-[")[0]
          )
      );
      if (passedClass) {
  ...
  const finalClasses = [
    ...new Set([...processedClassList, ...passedClassList]),
  ].join(" ");
  return finalClasses;
}
...
export default function Button({
  ...
  const defaultClassName =
    "container relative backdrop-blur-xs w-[160px] h-[60px] bg-[#4e4c57]/100 text-white shadow-[4px_4px_20px_rgba(0,0,0,0)]";

  // If className is passed, process and resolve conflicts
  const finalClassName = className
    ? resolveClassConflicts(defaultClassName, className)
    : defaultClassName;
  ...
  return (
    ...
    >
      <span
        className={`group glassbutton relative block w-full h-full no-underline text-sm tracking-wide overflow-hidden hover:text-[#298fef] group-hover:text-[#298fef] ${finalClassName}`}
      >
