...
const SnowflakesEffect = () => {
  useEffect(() => {
    const snowflakesContainer = document.createElement("div");
    snowflakesContainer.classList.add("absolute", "top-0", "left-0", "w-full", "h-full", "pointer-events-none");
    ...
    const createSnowflake = (x, y) => {
      const random = Math.random(0,4)+1
    ...
    const handleMouseMove = (e) => {
      const now = Date.now();
      if (now - lastTime > interval) {  // Check if enough time has passed since last snowflake
